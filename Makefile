export PDK_COMPATIBILITY_VERSION = 1.9.0
...
run: build ## Run the policy in local flex
	...
	@anypoint-cli-v4 pdk patch-gcl -f playground/config/api.yaml -p "spec.services.upstream.policies[0].policyRef.name" -v "$(POLICY_REF_NAME)"
	@anypoint-cli-v4 pdk patch-gcl -f playground/config/api.yaml -p "spec.services.upstream.policies[0].policyRef.namespace" -v "$(DEFINITION_NAMESPACE)"
ifeq ($(OS), Windows_NT)
